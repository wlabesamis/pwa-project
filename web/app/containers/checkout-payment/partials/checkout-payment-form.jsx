...
const validate = (values, savedAddressId, context) => {
    ...
    if (values.ccexpiry && !validateCCExpiry(values.ccexpiry)) {
        errors.ccexpiry = context && formatMessage(context, {id: 'checkoutPayment.form.validation.fixCVVExpiry'})
    }
    ...
    if (values.cvv) {
        ...
        if ((isAmex && cvvLength !== amexCvv) || (!isAmex && cvvLength !== defaultCvv)) {
            errors.cvv = context && formatMessage(context, {id: 'checkoutPayment.form.validation.fixCVVExpiryLength'})
        }
    }
